...
- **`go-dependency-submission.yml`** — Reusable. Submits the resolved Go dependency graph (`actions/go-dependency-submission@<sha>`) to the Dependency Submission API. Inputs: `go_mod_path` (default `go.mod`), `go_build_target` (empty omits the input so the action's own `all` default applies — split across two `if:`-gated steps). Secret `ci_read_app_private_key` (+ legacy/ignored `ci_read_app_id`) mints a ci-read App token (via `vars.CI_READ_APP_CLIENT_ID`) for an org-scoped (`github.com/nics-dp/` only) private-module git rewrite in a short-lived `$RUNNER_TEMP` gitconfig via `GIT_CONFIG_GLOBAL` + `GOPRIVATE` (mirrors `mise-task.yml`; the PAT model was retired org-wide). Perms: `contents: write`.
