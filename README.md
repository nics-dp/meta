...
> **`go:local` / `go:remote` 注意事項：** `go:local` 會自動停用 `.golangci.yml` 中的 `gomoddirectives` 規則，並建立 `.semgrepignore` 以排除 vendored local libs。`go:remote` 會還原這些變更。`.semgrepignore` 不應被 commit（`golang-templates/.gitignore` 已預設忽略）。
