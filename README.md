...
| `templates/facades/`   | mise.toml facade templates (`mise.go-service.toml`, `mise.go-lib.toml`, `mise.frontend.toml`, `mise.python.toml`, `mise.image.toml`) — copy 後依需求加 repo-specific 即可使用 |
...
| 類別      | Atoms                                                                                                                                                                                          | 用途                                                                                                                                                                                                                                                                                                                                                                                                |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci:*`    | semgrep, trivy-license, betterleaks, trufflehog, toolchain-pins                                                                                                                                | 通用 CI 檢查（語言中立）。`ci:semgrep` 引擎已改為 OpenGrep（原生 single-binary semgrep fork，無 Python；CLI/registry packs drop-in 相容，task 名與簽名不變）。`ci:betterleaks`＝快速 secret 掃描 PR gate（Gitleaks fork，PR 上優先掃 base..HEAD，base ref fetch 失敗則 fallback 全歷史）；`ci:trufflehog`＝deep/release scan，只報經驗證的 live secrets；`ci:toolchain-pins`＝檢查 go/python/node/bun 版本在 mise.toml、go.mod、Dockerfile、workflows `*_version`/`*-version`、.python-version、.nvmrc 間一致（與 Renovate preset 的 `<lang> toolchain` 群組共用這些主要來源），另檢查 repo-local `.mise/tasks` 內的映像參照（preset 無對應 manager，由 consumer 自備 manager 或人工更新，gate 只負責比對）；每個 pin 都必須是同一個完整 `X.Y.Z`，只到 `X.Y` 或 `X` 的短 pin 直接判失敗（SHORT）；README 一律不寫版本，只寫「版本見 `mise.toml`」 |
...
> **`.golangci.yml` 例外**：由 consumer repo **自家 commit**（非 curl），須含 `gofumpt.module-path: <repo-module>` 設定（golangci-lint 自動推導對無 `.` 模組名會誤判 stdlib，致 gci↔gofumpt import 互相 false positive）。範本直接 ref 既有 Go repo 之 `.golangci.yml`。
...
| Workflow             | 用途                                                                                                                       |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `go-release.yml`     | Go binary 跨平台建置 + cosign 簽章 + 多平台 SBOM；jobs：`prepare` → `build` (matrix) → `release`                          |
...
| `sbom-image.yml`     | Container image CycloneDX SBOM + parlay 增強 + Trivy/Grype 漏洞掃描 → GitHub Release + Security tab                        |
...
| `node-dependency-submission.yml`  | 預設送 Bun production npm 圖（含 optional、peer 與 transitive；排除 dev-only）；完整 frozen、ignore-scripts 安裝後，以原生 inventory 驗證版本／位置相依圖，保留非 npm 與第一方 workflow findings；封存校正後 SPDX；套件安裝不使用 token |
...
本機 `go:sast` atom 維持 blocking，gosec 版本釘在 `.mise/tasks/go/sast` 的 `#MISE tools` header 與 `security-sarif.yml` 的 `go install` 行（code is truth，文件不重複版本）；meta repo 自家的 Renovate custom manager 會同步更新這兩處 pin。
...
- `ci.yml` — meta repo 自家 CI。穩定 matrix check 為 `Actionlint`、`Mise Tasks`、`ShellCheck`；`mise run all` 仍會執行 `iac:zizmor`。ShellCheck 由 Git index 動態枚舉所有 tracked task/helper 與 scanner wrapper，只接受 stage 0、mode 100644/100755 的 regular non-symlink blobs，並以 Bash/warning threshold 檢查。Local Zizmor（版本以 `.mise/tasks/iac/zizmor` 的 `#MISE tools` pin 為準）使用 `--offline --strict-collection`、regular persona、high threshold，以及 `zizmor.yml` 內的 exact line dispositions（既有 App trust/cleanup boundary 與 protected `main` self-dogfood boundary；數量以該檔為準）。
- `Zizmor Action` 是獨立 hosted early-development gate，使用 official action 與 Zizmor 對應的 reviewed image digest（版本以 `ci.yml` 為準）、`online-audits: false`、high threshold，且不啟用 Advanced Security、SARIF 或 annotations。Action 會把公開、無權限且非 credential 的 compatibility placeholder 傳給 Zizmor process；不會提供 GitHub credential。Checkout 仍使用 job 的 `contents: read` token，但設有 `persist-credentials: false`。這不是完整離線或 network-isolated：container 仍可能有 residual egress，raw CLI output 也會留在無 secrets 的 job log；final PR CI 是 action/container runtime acceptance。它取代舊 matrix `Zizmor` check name，但不取代 local `iac:zizmor` gate。
- `Grype Path (high, fixed)` 使用 `anchore/scan-action` 與 Grype（版本以 `ci.yml` 為準），nested exact-SHA checkout、trusted mode-0600 config、`only-fixed` High/Critical blocking policy、無 cache/SARIF/upload。DB/tool download 或掃描失敗 fail closed；summary 只保留 sanitized counts/metadata，config/index/JSON cleanup 失敗亦 blocking。Medium 與 unfixed findings 刻意不在此 gate。
- `GitHub Actions Scanner (experimental)` 固定 Snyk Labs signed commit 與 lock digest（值以 `.github/scripts/github-actions-scanner.sh` 的 `SCANNER_SHA` / `SCANNER_LOCK_SHA256` 為準，且必須與 `ci.yml` 的 checkout `ref` 及 `Verify scanner source` step 的 `expected_sha` / `expected_lock` 同步，否則驗證 step 失敗），tokenless `npm ci --ignore-scripts` 後只跑 wrapper `RULES` 列出的 production rules；目前的 finding 基線看 job summary，不寫進文件。finding 與任何 network/rule/output failure 都是 informational，後者明確標成 `incomplete`，永不偽裝成 `clean`。Scanner identity 固定為 `${{ github.repository }}` + `${{ github.sha }}`；只有非 Dependabot 的 push/workflow_dispatch，或同 repo、非 Dependabot user 且 association 為 OWNER/MEMBER/COLLABORATOR 的 PR，才把 current-repo read token 限定給 Node scanner child。Fork、Dependabot、其他 association/event 一律 tokenless `incomplete`，不 fallback。Raw scanner output/token 不會進 summary；source/cache/results cleanup 失敗 blocking。此 unsupported research scanner 的 commit 只允許 manual source/support/lock/baseline review，不由 Renovate 更新。
...
**`.golangci.yml` 例外**：不在 `configs/` 共享。各 Go consumer repo **自家 commit** `.golangci.yml`（含 `gofumpt.module-path: <module>` 顯式設定，繞過 golangci-lint 之 gci↔gofumpt false positive）。`go:lint-check` / `go:lint-fix` atom **不再 curl**，直接讀 repo 自家檔。
...
`renovate-preset.json` 以 native managers（`github-actions`、`gomod`、`dockerfile`、`mise` 等）為主，另加 regex customManagers 處理 native manager 辨識不到的釘版（清單以 `renovate-preset.json` 為準）：
