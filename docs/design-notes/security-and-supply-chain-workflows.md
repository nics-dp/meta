...
- **`node-dependency-submission.yml`** — Submits production npm dependencies by default, including optional and peer routes, while preserving non-npm packages and first-party workflow findings. Inputs remain `working_directory` (default `.`) and `bun_version` (the workflow holds the measured default pin). Full `bun install --frozen-lockfile --ignore-scripts` runs in a private temporary project containing the manifest, lockfile, and declared patch files. Patch paths must be relative, contained regular files and cannot introduce package-manager configuration. Manifest, lockfile, and patch hashes must remain unchanged through staging and installation. Native `bun pm licenses --prod --json` independently gates the reconstructed graph's unique name/resolution identities; its representative paths are validated but are not treated as a complete occurrence list. Syft scans with development dependencies included and excludes only bundled `node_modules` workflow files; npm filtering happens after discovery. Corrected native/SPDX graphs retain occurrence-level relationships and cross-ecosystem edges. The final GitHub manifest aggregates npm occurrences by canonical PURL, unions their dependency edges, and marks an identity direct when any occurrence is a production root. Full package URLs retain source qualifiers; inconsistent name/resolution or conflicting source qualifiers fail closed. The artifact contains corrected SPDX, not the development inventory. Package processing receives no GitHub token; only the pinned publication action uses the job's `contents: write` permission.

  All Bun and Syft processes use an allowlisted environment, private home/config/temp roots, and trusted explicit configuration. Checkout and installed-tree paths must stay within their canonical roots. Local `file:`, `link:`, and workspace lock resolutions fail closed; each project has its own manifest and text lockfile. Native warnings, unresolved required edges, missing scanner records, conversion errors, and inventory differences prevent publication. Syft's exact unconditional experimental-conversion notice is informational; other scanner/converter warnings remain fatal. Publication uses an atomically renamed validated directory and a digest receipt. The PR-event allowlist selects the head SHA for `pull_request`, `pull_request_comment`, `pull_request_review`, and `pull_request_review_comment`; other events, including `pull_request_target`, use the context SHA. Checkout HEAD must equal that SHA. Detector identity and the emoji-stripped `${workflow}_${job}` correlator remain stable.

  `.github/tests/test_node_dependency_submission.py` extracts the actual inline helper. CI reads the workflow's tool pins and runs its non-execution and conversion checks. For no-submit integration, extract with `python3 -I -B .github/tests/test_node_dependency_submission.py --extract "$STATE/helper.py"`, then run `python3 -I "$STATE/helper.py" finish --state "$STATE" --project "$PROJECT" --source "$SCAN_ROOT" --raw "$RAW_SYFT_JSON" --inventory "$BUN_INVENTORY_JSON" --inventory-stderr "$BUN_INVENTORY_STDERR" --context "$CONTEXT_JSON" --syft "$SYFT"`. `STATE` is a fresh private temporary directory outside the scan root; `PROJECT` is the installed tree named by native inventory paths; `SCAN_ROOT` matches the raw Syft directory source. The stderr file must exist and be empty. Context JSON supplies `sha`, `ref`, `repository`, `workflow`, `job`, `run_id` (string), and repository-relative `working_directory`. This command performs no installation or submission; it writes `corrected.syft.json` and, only after successful conversion and validation, `published/snapshot.json`, a digest receipt, and the repository/job-named SPDX artifact.
