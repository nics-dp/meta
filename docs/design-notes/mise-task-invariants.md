...
4. Task-level tools in the atoms (`#MISE tools=`) are pinned to **exact** versions, never `latest`, and bumped by Renovate: the org preset's mise task-header managers cover `aqua:`/`github:`/bun pins, and `renovate.json`'s custom managers cover what those cannot see (core tools such as trivy and grype, `go:` modules, the workflows' `go install` lines). Resolving `latest` for a Go-backend tool lists every tag from the module proxy inside mise's remote-versions timeout, and on timeout mise only warns and runs the task without the tool (`command not found`); an exact version resolves with no listing. `MISE_FETCH_REMOTE_VERSIONS_TIMEOUT` in `mise-task.yml` covers any remaining `latest`, and `.mise/tasks/lib/require-tool` fails an atom at the top when its tool is missing. Scanner freshness is a run-time property (trivy-db, grype-db, the opengrep registry); the `security scanners` rules in `renovate.json` hold the release-age buffer and the automerge schedule.
